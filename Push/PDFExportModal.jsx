...
const PDFExportModal = ({ show, onClose, allRegistrants }) => {
    ...
    const stats = useMemo(() => {
        ...
        filtered.forEach(reg => {
            reg.crops?.forEach(c => {
                if (c.name) {
                    // Normalize name match? For now assume exact or substring match if key exists
                    // Or just accumulate all names found, but ensure standard ones are initialized
            ...
            reg.livestock?.forEach(l => {
                // Determine animal name: 'name' or from options
                // The provided 'livestockOptions' are broad types. The data might have 'Pig' or 'Swine'.
                // We'll trust the data's 'name' or 'animal_type' property.
                // Assuming data structure has 'name' or 'animal'.
                // The Dashboard code used `l.animal`. Let's check `l.name`.
                // For now, accumulation:
                const name = l.animal || l.name;
                if (name) {
                    const key = livestockOptions.find(opt => opt.toLowerCase().includes(name.toLowerCase())) || name;
                    // Note: using head_count for animals? Modal previously used count of records.
                    // User asked for "Value". Usually head count for animals.
                    // But here we are counting *records* (farmers who have pigs) or *heads*?
                    // Previous implementation counted records `(counts.livestock[l.name] || 0) + 1`.
                    // Dashboard sums `head_count`.
                    // Let's stick to record count for the FILTER UI (how many rows match), 
                    // but the Report will show Head Count.
                    // WAIT. The Checkbox shows "Pig (12)". Is that 12 farmers or 12 pigs?
                    // Usually filter counts show number of MATCHES (Registrants).
                    // But user wants "Total Value".
                    // Let's stick to occurrence for filter count (simpler) or switch to head count?
                    // Let's keep occurrence for consistency with "Matching Registrants".
    ...
    const handleDownload = () => {
        ...
        const config = {
            ...
            registryType, // Pass the type 'none', 'all', 'farmer' etc
            reportType,
            previewMode: false
        };
